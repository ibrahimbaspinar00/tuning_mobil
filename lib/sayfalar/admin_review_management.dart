...
class _AdminReviewManagementState extends State<AdminReviewManagement> {
  ...
  Future<void> _approveReview(ProductReview review) async {
    try {
      ...
      if (mounted) {
        if (success) {
          ErrorHandler.showSuccess(context, 'Yorum onaylandı');
          _loadReviews();
        } else {
          ErrorHandler.showError(context, 'Yorum onaylanırken hata oluştu');
        }
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _rejectReview(ProductReview review) async {
    try {
      ...
      if (mounted) {
        if (success) {
          ErrorHandler.showSuccess(context, 'Yorum reddedildi');
          _loadReviews();
        } else {
          ErrorHandler.showError(context, 'Yorum reddedilirken hata oluştu');
        }
      }
    } catch (e) {
      if (mounted) {
